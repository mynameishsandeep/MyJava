...
public class RemoveInvalidParentheses {
	...
	public void dfs(String prefix, String suffix, int countOpenParentheses, int countCloseParentheses) {
		//System.out.println(prefix + suffix);
		if (prefix.length() == 0) { // Reached end of string
			if (countOpenParentheses == countCloseParentheses) {
				if (countOpenParentheses + countCloseParentheses >= maxResultSize) {// we want multiple result with same size. So >=
		...
		//System.out.println(prefix + suffix);
		if (prefix.charAt(0) == '(') {
			dfs(prefix.substring(1), suffix + "(", countOpenParentheses + 1, countCloseParentheses);//keep (
			...
			dfs(prefix.substring(1), suffix + String.valueOf(prefix.charAt(0)), countOpenParentheses,
